...
struct _L6_data {
	...
	uint8_t	RSA_enc_msg[2048];
	uint8_t	RSA_enc_msg0_sig[2048];
	uint8_t	RSA_enc_msg1_sig[2048];
...
int main(void)
{
	...
	// ---------------------------- IN: datele cu adresa hash-urilor ----------------------------
	to_hash.buffer = (uint8_t*)&L6_data;
	to_hash.length = 256 * 4 + 32;

	// ---------------------------- OUT: Se initializeaza cu adresa sign-out ----------------------------
	hash_out = &L6_data.sig_sig;

	// ============================ Se semneaza cele 4 mesaje ============================
	optiga_lib_status = OPTIGA_LIB_BUSY;
